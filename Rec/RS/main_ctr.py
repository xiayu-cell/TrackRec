...
from models import DeepInterestNet, DCN, DeepFM, DIEN, xDeepFM, FiBiNet, FiGNN, AutoInt, DNN
from dataset import AmzDataset, EcomDataset, MovieCoLLMDataset
...
def eval(model, test_loader):
    ...
    acc_preds = [1 if item[0] >= 0.5 else 0 for item in preds]
    acc = accuracy_score(y_true=labels, y_pred=acc_preds)
    return auc, ll, np.mean(losses), eval_time, acc
...
def train(args):
    if "ml-1m" in args.data_dir:
        train_set = MovieCoLLMDataset(args.data_dir, 'train', args.task, args.max_hist_len, args.augment, args.aug_prefix)
        test_set = MovieCoLLMDataset(args.data_dir, 'test', args.task, args.max_hist_len, args.augment, args.aug_prefix)
    elif "amz" in args.data_dir:
        train_set = AmzDataset(args.data_dir, 'train', args.task, args.max_hist_len, args.augment, args.aug_prefix)
        test_set = AmzDataset(args.data_dir, 'test', args.task, args.max_hist_len, args.augment, args.aug_prefix)
    #train_set = EcomDataset(args.data_dir, 'train', args.task, args.max_hist_len, args.augment, args.aug_prefix)
    #test_set = EcomDataset(args.data_dir, 'test', args.task, args.max_hist_len, args.augment, args.aug_prefix)
    train_loader = Data.DataLoader(dataset=train_set, batch_size=args.batch_size, shuffle=True, num_workers=16)
    ...
    for epoch in range(args.epoch_num):
        ...
        eval_auc, eval_ll, eval_loss, eval_time, eval_acc = eval(model, test_loader)
        print("EPOCH %d  STEP %d train loss: %.5f, train time: %.5f, test loss: %.5f, test time: %.5f, auc: %.5f, "
              "logloss: %.5f, acc: %.5f" % (epoch, global_step, np.mean(train_loss), train_time, eval_loss,
                                 eval_time, eval_auc, eval_ll, eval_acc))
...
def parse_args():
    ...
    parser.add_argument('--device', default='cuda:1' if torch.cuda.is_available() else 'cpu', type=str, help='device')
