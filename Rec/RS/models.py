...
class BaseModel(nn.Module):
    def __init__(self, args, dataset):
        ...
        #self.augment_num = 1 if args.augment else 0
        ...
        self.item_fnum = 1 + self.attr_fnum # 2 or 128
        self.hist_fnum = 2 + self.attr_fnum
        #self.item_fnum = 5 + 1
        self.user_fnum = 1
        #self.hist_fnum = 2 + 1
        self.itm_emb_dim = self.item_fnum * self.embed_dim
        self.user_emb_dim = self.user_fnum * self.embed_dim
        ...
        self.user_embedding = nn.Embedding(self.user_num + 1, self.embed_dim)
    ...
    def process_input(self, inp):
        ...
        """
        # cot emb split
        split_hist_attr_emb = torch.split(hist_attr_emb, (2 * self.embed_dim, (self.attr_fnum - 2) * self.embed_dim), 2)
        hist_attr_emb = split_hist_attr_emb[0]
        hist_cot_emb = split_hist_attr_emb[1].reshape(-1, self.max_hist_len, (self.attr_fnum - 2), self.embed_dim).mean(-2)
        hist_cot_emb = split_hist_attr_emb[1].reshape(-1, self.max_hist_len, 10, int((self.attr_fnum - 2) / 10), self.embed_dim)
        hist_cot_emb = hist_cot_emb.mean(-2).reshape(-1, self.max_hist_len, 10 * self.embed_dim)
        """
        ...
        #hist_emb = torch.cat([hist_item_emb, hist_attr_emb, hist_cot_emb, hist_rating_emb], dim=-1)
        ...
        if self.task == 'ctr':
            ...
            #attr_emb = self.attr_embedding(inp['aid'].to(device)).view(-1, self.embed_dim * 5)
            #user_attr_emb = self.attr_embedding(inp['uaid'].to(device)).view(-1, self.embed_dim * 10)
            uid_emb = self.user_embedding(inp['uid'].to(device))
            """
            split_attr_emb = torch.split(attr_emb, (2 * self.embed_dim, (self.attr_fnum - 2) * self.embed_dim), 1)
            attr_emb = split_attr_emb[0]
            #cot_emb = split_attr_emb[1].reshape(-1, (self.attr_fnum - 2), self.embed_dim).mean(-2)
            cot_emb = split_attr_emb[1].reshape(-1, 10, int((self.attr_fnum - 2) / 10), self.embed_dim).mean(-2).reshape(-1, 10 * self.embed_dim)
            #item_emb = torch.cat([iid_emb, attr_emb, cot_emb], dim=-1)
            """
            item_emb = torch.cat([iid_emb, attr_emb], dim=-1)
            user_emb = torch.cat([uid_emb], dim=-1)
            #user_emb = torch.cat([uid_emb, user_attr_emb], dim=-1)
            # item_emb = item_emb.view(-1, self.itm_emb_dim)
            labels = inp['lb'].to(device)
            if self.augment_num:
                orig_dens_vec = [inp['input_aug_vec'].to(device), inp['output_aug_vec'].to(device)]
                #orig_dens_vec = [inp['input_aug_vec'].to(device)]
    ...
    def dnn_process_input(self, inp):
        device = next(self.parameters()).device
        hist_item_emb = self.item_embedding(inp['hist_iid_seq'].to(device)).view(-1, self.max_hist_len, self.embed_dim)
        hist_attr_emb = self.attr_embedding(inp['hist_aid_seq'].to(device)).view(-1, self.max_hist_len,
                                                                                    self.embed_dim * self.attr_fnum)
        ...
        # # cot emb split
        # split_hist_attr_emb = torch.split(hist_attr_emb, (1 * self.embed_dim, (self.attr_fnum - 1) * self.embed_dim), 2)
        # hist_attr_emb = split_hist_attr_emb[0]
        # #hist_cot_emb = split_hist_attr_emb[1].reshape(-1, self.max_hist_len, (self.attr_fnum - 2), self.embed_dim).mean(-2)
        # hist_cot_emb = split_hist_attr_emb[1].reshape(-1, self.max_hist_len, 10, int((self.attr_fnum - 1) / 10), self.embed_dim)
        # hist_cot_emb = hist_cot_emb.mean(-2).reshape(-1, self.max_hist_len, 10 * self.embed_dim)
        ...
        if 'ctr' in self.task:
            ...
            # split_attr_emb = torch.split(attr_emb, (1 * self.embed_dim, (self.attr_fnum - 1) * self.embed_dim), 1)
            # attr_emb = split_attr_emb[0]
            # #cot_emb = split_attr_emb[1].reshape(-1, (self.attr_fnum - 2), self.embed_dim).mean(-2)
            # cot_emb = split_attr_emb[1].reshape(-1, 10, int((self.attr_fnum - 1) / 10), self.embed_dim).mean(-2).reshape(-1, 10 * self.embed_dim)
            # #item_emb = torch.cat([iid_emb, attr_emb, cot_emb], dim=-1)

            #attr_emb = self.attr_embedding(inp['aid'].to(device)).view(-1, self.embed_dim * 5)
            
            #user_attr_emb = self.attr_embedding(inp['uaid'].to(device)).view(-1, self.embed_dim * 15)
            uid_emb = self.user_embedding(inp['uid'].to(device))
            ...
            labels = inp['lb'].to(device)
            if self.augment_num:
                #orig_dens_vec = [inp['hist_aug_vec'].to(device).detach(), inp['item_aug_vec'].to(device).detach()]
                #orig_dens_vec = [inp['input_aug_vec'].to(device), inp['output_aug_vec'].to(device)]
                orig_dens_vec = [inp['output_aug_vec'].to(device)]
                # orig_dens_vec = [inp['input_aug_vec'].to(device)]
                # print(orig_dens_vec[0].shape, orig_dens_vec[1].shape)
                dens_vec = torch.cat(orig_dens_vec, dim=1)
                # print(dens_vec.shape)
                # exit()
                #dens_vec = self.convert_module(orig_dens_vec)
            else:
                dens_vec, orig_dens_vec = None, None
            #return item_emb, hist_emb, hist_len, dens_vec, orig_dens_vec, labels
            return item_emb, user_emb, dens_vec, orig_dens_vec, labels
...
class DNN(BaseModel):
    ...
    def get_input_dim(self):
        # with open('../data/amz/proc_data/stat.json', "r") as f:
        #      self.dens_vec_num = 2 * json.load(f)['dense_dim']
        # return self.itm_emb_dim + self.hist_emb_dim + self.dens_vec_num 
        #return self.itm_emb_dim + self.hist_emb_dim + self.dens_vec_num
        return self.itm_emb_dim + self.user_emb_dim  # + 1 * 1024
    ...
    def forward(self, inp):
        """
            ...
            :return score (bs)
        """
        #query, user_behavior, hist_len, dens_vec, orig_dens_vec, labels = self.dnn_process_input(inp)
        query, user, dens_vec, orig_dens_vec, labels = self.dnn_process_input(inp)
        ...
        if self.augment_num:
            ...
            concat_input = torch.cat([user, query, dens_vec], dim=-1)
        else:
            #concat_input = torch.cat([user_behavior, query], dim=-1)
            concat_input = torch.cat([user, query], dim=-1)
