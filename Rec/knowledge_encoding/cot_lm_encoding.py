...
def load_base_klg(path):
    res = {}
    with open(path, 'r', encoding='utf-8') as f:
        data = json.load(f)
        for id, value in data.items():
            #res.append([id, value['input'], value['label_answer']])
            res[id] = [value['prompt'], value['ans']]
    return res
...
def load_data(path):
    ...
    with open(path, 'r', encoding='utf-8') as f:
        for line in f.readlines():
            ...
            res.append([data['id'], data['label_answer']])
    return res
...
def get_history_text(data_path):
    ...
    item_klg = load_base_klg(klg_path + '/item.klg')
    user_klg = load_base_klg(klg_path + '/user.klg')
    idx_list, hist_text = [], []
    for piece in raw_data:
        idx, answer = piece
        pure_hist = user_klg[idx][0][::-1].split(';', 1)[-1][::-1]
        answer = pure_hist + '. ' + answer
...
def get_text_data_loader(data_path, batch_size):
    hist_idxes, history = get_history_text(os.path.join(data_path, 'prompt.hist.mmu.gpt35_output'))
    print('chatgpt.hist 1', history[1], 'hist len', len(history))
    item_idxes, items = get_item_text(os.path.join(data_path, 'prompt.item.mmu.gpt35_output'))
...
def remap_hist(hist_idxes, hist_vec):
    hist_vec_map = {}
    for idx, vec in zip(hist_idxes, hist_vec):
        if idx not in hist_vec_map.keys():
            hist_vec_map[idx] = []
        hist_vec_map[idx].append(vec)
    for idx in hist_vec_map.keys():
        hist_vec_list = hist_vec_map[idx]
        hist_vec_pooling = [float(item) for item in np.mean(np.array(hist_vec_list), axis=0).tolist()]
        hist_vec_map[idx] = hist_vec_pooling
    return hist_vec_map
...
def main(knowledge_path, data_path, model_name, batch_size, aggregate_type):
    ...
    if model_name == 'chatglm':
        checkpoint = '../../llm/chatglm-6b' if os.path.exists('../../llm/chatglm-6b') else 'THUDM/chatglm-6b'
        ...
        checkpoint = '../../llm/longformer-base-4096' if os.path.exists('../../llm/chatglm-6b') else 'allenai/longformer-base-4096'
    else:
    ...
    hist_vec_dict = remap_hist(hist_idxes, hist_vec)

    save_json(item_vec_dict, os.path.join(data_path, '{}_{}_v3_augment.item'.format(model_name, aggregate_type)))
    save_json(hist_vec_dict, os.path.join(data_path, '{}_{}_v3_augment.hist'.format(model_name, aggregate_type)))
...
if __name__ == '__main__':
    ...
    DATA_SET_NAME = 'ml-1m'
    #KLG_PATH = os.path.join(DATA_DIR, DATA_SET_NAME, 'knowledge')
    KLG_PATH = os.path.join(DATA_DIR, DATA_SET_NAME, 'proc_data')
    ...
    BATCH_SIZE = 16 if MODEL_NAME == 'bert' else 16
