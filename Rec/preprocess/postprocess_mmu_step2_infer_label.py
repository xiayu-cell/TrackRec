...
attribute_ft_num = statis['attribute_ft_num'] + 16 * 10
err_cnt = 0
                    ...
                    for content in contents:
                        ...
                        for c in content:
                            ...
                            if c in stop_words or c == '':
                                continue
                            tmp_close_content_list.append(c)
                    if len(tmp_close_content_list) >= 16:
                        tmp_close_content_list = tmp_close_content_list[0:16]
                    else:
                        tmp_close_content_list.extend(["null"] * (16 - len(tmp_close_content_list)))
                    close_label_list = close_label_list + label +'#'
                close_content_list.extend(tmp_close_content_list)
            if len(close_content_list) < 16 * 10:
                close_content_list.extend(["null"] * (16 * 10 - len(close_content_list)))
        else:
