...
def preprocess(data_file, test_file, processed_dir, data_type='Amazon'):
    ...
    for filename in os.listdir(data_file):
        if filename.startswith('infer_data_') and filename.endswith(".json"):
    ...
    for data in merged_data:
        ...
        user_features = user_profile.split('。')[0].replace("用户的基本画像如下：", "").split(',')
        ...
        user_attrs[0] = user_attrs[0].replace('平常喜欢浏览', '')
        user_attrs = pad_list(user_attrs)
        user_dict[user_profile] = (user_features + user_attrs)

        item_profile = data['pos_item'].replace("所提供商品的信息如下:", "")
        item_profile = item_profile.replace("。", "").replace("\n", "")
        train_user_items.append((user_profile, item_profile, 1))
        idx += 1
        ...
        item_profile = data['neg_item'].replace("所提供商品的信息如下:", "")
        item_profile = item_profile.replace("。", "").replace("\n", "")        
        train_user_items.append((user_profile, item_profile, 0))
        idx += 1

    user_set = list(user_dict.keys())
    train_user = user_set[:int(len(user_set) * 0.8)]
    test_user = user_set[int(len(user_set) * 0.8):]
...
if __name__ == '__main__':
    set_seed(1234)
    DATA_DIR = '/mmu_vcg2_wjc_ssd/gaoxiaoyang/aliyun/cot_paper/LC-Rec/ecom_mcpg_data/20241225'
    DATA_SET_NAME = 'ecom'
