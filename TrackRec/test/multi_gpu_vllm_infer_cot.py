...
def run_inference(model_path,batch, bs, thread_id):
    ...
    # 初始化 VLLM 模型，指定 GPU
    sampling_params = SamplingParams(temperature=0.0,top_p=0.8, repetition_penalty=1.05, max_tokens=1024)
    ...
    anss = [item['completion'] for item in batch]
    ...
    with open(f'{save_infer_n_dir}/model_A_cot_iter_{iter}_{thread_id}.json', 'w') as f:
        for i in tqdm(range(0,len(prompts),bs)):
            ...
            ans = anss[i:min(i+bs,len(prompts))]
            # 进行批量推理
